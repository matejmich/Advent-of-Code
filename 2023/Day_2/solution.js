...
const test2 = `Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
Game 2: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue
Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red
Game 4: 1 green, 3 red, 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green`
...
function firstPart (input) {
    ...
    for (let i = 0; i < lines.length; i++) {
        sumOfId += oneLine(lines[i])

    }
    console.log("sum of ID: ", sumOfId)
...
function oneLine(line) {
    ...
    for (j = 0; j < games.length; j++) {
        if( gamePossible) {
            gamePossible = checkIfPossible(games[j])
        }
        checkIfPossible(games[j])

    }
    // console.log("game: ", id, gamePossible)
    console.log("ID", id)
   
    powerOfGame += (blueNum * redNum * greenNum)
    console.log("power of a game: ", (blueNum * redNum * greenNum))
    console.log( "blue", blueNum, "red", redNum, "green", greenNum)
...
function checkIfPossible(game) {
    let move = game.trim()
    let throws = move.split(",")
    let possibile = true
    
    
    for (k = 0; k < throws.length; k++) {
        if(possibile) {
            possibile = isThrowPossible(throws[k])
        } 
        isThrowPossible(throws[k])
...
function isThrowPossible(hod) {
    hod = hod.trim()
    let num = parseFloat(hod.charAt(0) + hod.charAt(1))
    hod = hod.slice(2)
    let color = hod.trim()
    console.log( num, color)

    
    if (color === "blue" && num > blueNum) {
        blueNum = num
    console.log( "new blue", blueNum)

    }
    if (color === "red" && num > redNum) {
        redNum = num
        console.log( "new red", redNum)


    }
    if (color === "green" && num > greenNum) {
        greenNum = num
        console.log( "new green", greenNum)


    }
    console.log("---------")
    if (color === "blue" && num < 15) {
        return true
    } else if (color === "red" && num < 13) {
        return true
    } else if (color === "green" && num < 14) {
        return true
    } else {
        return false
    }
